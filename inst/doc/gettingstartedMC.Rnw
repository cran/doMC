...
and the \texttt{multicore} package, which was written by Simon Urbanek.
\texttt{multicore} currently only works with operating systems that
support the \texttt{fork} system call (which means that Windows isn't
supported).\footnote{At the time of this writing, there is an
experimental version of \texttt{multicore} that supports Windows, but it
isn't stable yet.}  Also, \texttt{multicore} only runs tasks on a single
...
\texttt{doNWS} is available from REvolution Computing:
\url{http://www.revolution-computing.com/products/parallel-r.php}.} That
...
Because the \texttt{multicore} package starts its workers using
